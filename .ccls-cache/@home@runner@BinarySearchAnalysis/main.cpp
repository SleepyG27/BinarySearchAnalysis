...
int recursiveBinarySearch(const vector<int> &vec, int left, int right,
                          int target) {
  if (left <= right) {
...
int main() {
  ...
  cout << "Recursive Binary Search: Target " << target1 << " found at location "
       << index << endl;

  index = recursiveBinarySearch(vec, 0, vec.size() - 1, target2);
  cout << "Recursive Binary Search: Target " << target2
       << " was not found, return value is " << index << endl;

  // Iterative Binary Search
  index = iterativeBinarySearch(vec, target1);
  cout << "Iterative Binary Search: Target " << target1 << " found at location "
       << index << endl;

  index = iterativeBinarySearch(vec, target2);
  cout << "Iterative Binary Search: Target " << target2
       << " was not found, return value is " << index << endl;

  // Sequential Search
  index = sequentialSearch(vec, target1);
  cout << "Sequential Search: Target " << target1 << " found at location "
       << index << endl;

  index = sequentialSearch(vec, target2);
  cout << "Sequential Search: Target " << target2
       << " was not found, return value is " << index << endl;
